...
class QFunction(nn.Module):
    ...
    def forward_replan(self, rgb_pcd, depth, proprio, pcd, camera_extrinsics, camera_intrinsics, lang_goal_emb,
                       ...
                       pcd_next=None, nerf_next_target_rgb=None, nerf_next_target_pose=None,
        ...
        if use_neural_rendering:  # train default: True; eval default: False
            ...
            if nerf_target_rgb is not None:
                ...
                depth_0 = depth[0]
                pcd_0 = pcd[0]
                pcd_next_flat_0 = pcd_next_flat[0] if pcd_next_flat is not None else None
    ...
    @torch.no_grad()
    def render_replan(self, rgb_pcd, proprio, pcd,
                      lang_goal_emb, lang_token_embs,
        ...
        # prepare nerf rendering
        # We only use the front camera
        _, ret_dict = self._neural_renderer.forward_replan(
            pcd=pcd[0],
            rgb=rgb[0],
            dec_fts=voxel_grid_feature,
            language=lang_embedd,
        ...
        return ret_dict.render_next_points
...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict, fabric: Fabric) -> dict:
        ...
        if self.use_neural_rendering:  # eval default: False; train default: True
            ...
            loss_rec_item = rendering_loss_dict['loss_rec']
            ...
            lambda_dyna = (
                              self.cfg.neural_renderer.lambda_dyna if step >= self.cfg.neural_renderer.next_mlp.warm_up else 0.) * lambda_nerf  # 0.01
            lambda_rec = (
                             self.cfg.neural_renderer.lambda_rec if step >= self.cfg.neural_renderer.next_mlp.warm_up else 0.) * lambda_nerf  # 0.01
            ...
            if step % 10 == 0 and rank == 0:
                cprint(f'total L: {total_loss.item():.6f} | \
                    ...
                    L_col: {q_collision_loss.item():.6f}  | \
                    L_rgb: {loss_rgb_item:.6f}  | \
                    L_embed: {loss_embed_item:.6f}  | \
                    L_dyna: {loss_dyna_item:.6f}  | \
                    L_rec: {loss_rec_item:.6f}  | \
                    psnr_0: {psnr:.6f}', 'green')
    ...
    def act_replan(self, step: int, observation: dict, deterministic=False) -> ActResult:
        deterministic = True
        bounds = self._coordinate_bounds
        prev_layer_voxel_grid = None
        prev_layer_bounds = None
        prev_predict_points = observation.get('prev_layer_voxel_grid', None)
        prev_action = observation.get('prev_layer_bounds', None)
        lang_goal_tokens = observation.get('lang_goal_tokens', None).long()
        lang_goal = observation['lang_goal']
        ...
        proprio, current_gripper = None, None
        proprio = observation['low_dim_state'][:, :, 7:]
        current_gripper = observation['low_dim_state']
        # 3+4+1+2+1
        if self._include_low_dim_state_11:
            proprio = observation['low_dim_state']
        ...
        obs = [[o[0][0].to(self._device), o[1][0].to(self._device)] for o in obs]
        proprio = proprio[0].to(self._device)
        current_gripper = current_gripper[0]
        current_gripper = torch.cat([current_gripper[:, :8], current_gripper[:, 10:]], dim=-1)
        current_gripper = current_gripper[0]
        pcd = [p[0].to(self._device) for p in pcd]
        lang_goal_emb = lang_goal_emb.to(self._device)
        lang_token_embs = lang_token_embs.to(self._device)
        ...
        q_trans, \
            ...
            q_ignore_collisions, \
            vox_grid, \
            rendering_loss_dict = self._q.forward_replan(obs,
                                                         depth,
                                                         proprio,
                                                         pcd,
                                                         extrinsics,
        ...
        translation_results.append(coords)
        if rot_grip_action is not None:
            rot_grip_results.append(rot_grip_action)
        if ignore_collisions_action is not None:
            ignore_collisions_results.append(ignore_collisions_action)
        ...
        next_points = self._q.render_replan(
            ...
            lang_goal=lang_goal,
            step=step,
            action=torch.tensor(continuous_action[:8], dtype=torch.float32).unsqueeze(0).to(self._device),
        )
        current_points = pcd[0].permute(0, 2, 3, 1).reshape(1, -1, 3)

        cd_distance = 0.
        if prev_predict_points is not None:
            from agents.gengaussian_bc.loss import l1_loss, l2_loss, cosine_loss, ssim, cd_loss
            prev_predict_points = torch.tensor(prev_predict_points.astype(np.float32), dtype=torch.float32).to(
                self._device)
            cd_distance = cd_loss(prev_predict_points, current_points)
            cd_distance = cd_distance.item()

        prev_action = np.array(prev_action)
        if cd_distance > 0.40:
            # todo: add noise?
            pie = 1.
            perturbation = np.random.uniform(-0.001 * pie, 0.001 * pie, 3)
            prev_action[:3] += perturbation
            observation_elements = {
                'attention_coordinate': attention_coordinate,
                'prev_layer_voxel_grid': None,
                'prev_layer_bounds': None,
            ...
            observation_elements = {
                'attention_coordinate': attention_coordinate,
                'prev_layer_voxel_grid': next_points,
                'prev_layer_bounds': current_gripper,
                'action': None
            }
        info = {
            'voxel_grid_depth%d' % self._layer: vox_grid,
